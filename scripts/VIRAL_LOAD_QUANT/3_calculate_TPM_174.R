...
filtered_metadata <- metadata[grepl("_Mock_|_174_", metadata$Sample), ]

# Process each sample
for (i in 1:nrow(filtered_metadata)) {
  #strain_name <- ifelse(grepl("134", metadata$Sample[i]), "134", "174")
  strain_name <- ifelse(grepl("Mock", filtered_metadata$Sample[i]), "Mock", "174")
  sample_name <- filtered_metadata$Sample[i]

  combined_gtf_file <- "combined_pig_NC174.gtf"
  viral_gtf_file <- "NC174_annot.gtf"
  
  gtf_path <- sprintf("%s/raw_data/ref_genomes/PRRSV2/NC174/%s", base_dir, combined_gtf_file)
  viral_gtf_path <- sprintf("%s/raw_data/ref_genomes/PRRSV2/NC174/%s", base_dir, viral_gtf_file)
  ...
  file_path <- sprintf("%s/ongoing/mapping/PRRSV2_174_QUANT/PRRSV2_174_QUANT/%s/PRRSV2_174_QUANT_%s_ReadsPerGene.out.tab",
...
# Write to file
write.table(tpm_results, file=file.path(out_dir, "viral_load_tpm_results_NC174.tsv"), quote=FALSE, sep="\t", row.names=FALSE)
